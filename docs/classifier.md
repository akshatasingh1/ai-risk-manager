...
2. **Correction next** — class weighting (`scale_pos_weight`, applied only to the training data, never the holdout) so the model is pushed to actually learn the minority class.
...
| Metric | Baseline (Logistic Regression) | v2 (XGBoost, class-weighted) |
|---|---|---|
| Precision | 81.6% | 38.7% |
| Recall | 26.9% | **82.9%** |
| F1 | 0.40 | 0.53 |
...
Out of 4,133 actual fraud cases in the holdout, the baseline caught 1,111; v2 catches **3,426** — a large recall improvement, and PR-AUC (the metric that matters most for a ranking/scoring problem like this) improved from 0.47 to 0.76.

**Why precision dropped:** correcting for the imbalance (via `scale_pos_weight`) intentionally makes the model much more willing to flag something as fraud, which catches far more real fraud but also produces more false alarms (5,421 legitimate transactions flagged, vs. 251 for the baseline) — all measured at the same default 0.5 cutoff. This tradeoff is expected and by design: **the 0.5 cutoff is arbitrary and was never meant to be final.** The next step is to replace it with a cost-based threshold that explicitly balances "cost of a manual review" against "cost of missed fraud," rather than reading precision/recall at an untuned default.
...
**The classifier is now locked.** At the cost-optimal threshold of **0.83**:

| Metric | @ default 0.5 | @ cost-optimal 0.83 |
|---|---|---|
| Precision | 38.7% | **75.8%** |
| Recall | 82.9% | 65.9% |
| F1 | 0.53 | **0.71** |
| Manual reviews per real fraud caught | 1.58 | **0.32** |
| Total holdout cost | ₹667,551 | **₹337,421** |

Moving from the untuned 0.5 cutoff to the cost-optimal 0.83 cuts total cost by **~49%** (₹330,130 saved on the holdout alone) — and drops manual review burden from "1.58 reviews per fraud caught" to "0.32 reviews per fraud caught," meaning most flagged cases are now real fraud rather than false alarms. This confirms the general point: 0.5 was never the right number to judge the model at — PR-AUC (0.76) reflected the model's real ranking ability all along, and the threshold sweep is what turns that ranking ability into a good operating point.

This is also a real, honest trade-off, not a free lunch: recall drops from 82.9% to 65.9% — the system now misses more fraud than it did at 0.5, in exchange for far fewer false alarms. That trade-off is exactly what the ₹ cost assumptions above encode, and a different assumption (e.g. a higher missed-fraud cost, or a cheaper review cost) would shift the threshold and this trade-off differently — which is the point of making the costs explicit rather than baking in a silent judgment call.
...
- **`C13`, `C1`, `C11`, `C14`, `C2`, `C6`** — Vesta's "identity count" features: roughly, how many other accounts/cards/addresses are linked to this identity. The single strongest group of signals.
- **`TransactionAmt`** — the transaction amount itself
- **`card1`, `card2`, `card5`, `card6` (credit vs. debit)** — characteristics of the card used
- **`D1`, `D4`, `D10`, `D15`** — time-delta features: how long since related past activity (e.g. since this card was last seen)
- **`ProductCD` (category "R")** — the type of product/service purchased
- **`addr1`** — billing address region
- **A few `V` columns** (`V70`, `V258`, `V294`) — Vesta's anonymized engineered features; their exact real-world meaning isn't disclosed by the dataset provider, but they're empirically influential
